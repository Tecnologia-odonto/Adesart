/*
  # Corrigir filtro de vendedor nas estatísticas para usar vendedor_id
  
  1. Problema
    - get_cadastros_stats estava usando vendedor_codigo = external_id para filtrar cadastros de vendedor
    - Isso causava inconsistência pois nem todos os cadastros têm vendedor_codigo preenchido
    - O correto é usar vendedor_id = profile.id que é sempre preenchido
  
  2. Solução
    - Alterar filtro de (vendedor_codigo = v_user_external_id OR created_by = p_user_id)
    - Para (vendedor_id = p_user_id OR created_by = p_user_id)
  
  3. Benefícios
    - Garante que todos os cadastros criados pelo vendedor sejam contados
    - Mantém consistência com a lógica de RLS que usa vendedor_id
*/

-- Dropar função anterior
DROP FUNCTION IF EXISTS get_cadastros_stats(uuid);

-- Recriar função com filtro correto usando vendedor_id
...
-- Comentários
COMMENT ON FUNCTION get_cadastros_stats IS 'Retorna estatísticas detalhadas de cadastros do mês atual separadas por tipo_cadastro. Para vendedores, usa vendedor_id (sempre preenchido) ao invés de vendedor_codigo.';