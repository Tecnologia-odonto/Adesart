/*
  # Corrigir agrupamento de estatísticas por vendedor_id
  
  1. Problema
    - A função get_cadastros_stats_by_vendedor estava agrupando por created_by
    - Isso mostra todos os usuários que criaram cadastros, não apenas os vendedores
    - O correto é agrupar por vendedor_id que é o campo que identifica o vendedor do cadastro
  
  2. Solução
    - Modificar agrupamento de created_by para vendedor_id
    - Buscar nome do vendedor através de profiles usando vendedor_id
    - Manter filtros de visibilidade por role (ADMINISTRADOR, GERENTE, SUPERVISOR)
  
  3. Benefícios
    - Mostra apenas vendedores que têm cadastros associados
    - Reflete corretamente o vendedor responsável pelo cadastro
    - Consistente com o campo vendedor_id que sempre deve estar preenchido
*/

-- Dropar função anterior
DROP FUNCTION IF EXISTS get_cadastros_stats_by_vendedor(uuid, text);

-- Recriar função agrupando por vendedor_id
...
BEGIN
  ...
  IF v_user_role IN ('ADMINISTRADOR', 'GERENTE') THEN
    ...
    FROM (
      SELECT
        c.vendedor_id,
        COALESCE(p.name, c.vendedor_nome, 'Vendedor não identificado') as vendedor_nome,
      ...
      GROUP BY c.vendedor_id, p.name, c.vendedor_nome
      ORDER BY COALESCE(p.name, c.vendedor_nome)
    ) stats;
...
-- Comentários
COMMENT ON FUNCTION get_cadastros_stats_by_vendedor IS 'Retorna estatísticas de cadastros do mês atual agrupadas por vendedor_id (campo que identifica o vendedor responsável), filtradas por tipo_cadastro. Apenas ADMINISTRADOR, GERENTE e SUPERVISOR podem acessar.';
