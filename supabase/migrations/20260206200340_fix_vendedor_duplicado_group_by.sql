/*
  # Corrigir duplicação de vendedores nas estatísticas
  
  1. Problema
    - A função estava agrupando por: vendedor_id, p.name, c.vendedor_nome
    - O campo c.vendedor_nome pode ter valores diferentes para o mesmo vendedor_id
    - Isso causa múltiplos grupos para o mesmo vendedor (duplicação)
    
  2. Exemplos de duplicação
    - vendedor_id='123', vendedor_nome='João Silva'
    - vendedor_id='123', vendedor_nome='JOÃO SILVA'  
    - vendedor_id='123', vendedor_nome=NULL
    - Esses três geram 3 linhas para o mesmo vendedor!
  
  3. Solução
    - Agrupar APENAS por vendedor_id
    - Usar MAX(p.name) para pegar o nome da tabela profiles
    - Usar MAX(c.vendedor_nome) como fallback
    - Isso garante um único registro por vendedor
*/

-- Dropar função anterior
DROP FUNCTION IF EXISTS get_cadastros_stats_by_vendedor(uuid, text);

-- Recriar função com agrupamento correto
...
-- Comentários
COMMENT ON FUNCTION get_cadastros_stats_by_vendedor IS 'Retorna estatísticas de cadastros do mês atual agrupadas por vendedor_id único. Evita duplicações causadas por variações no nome do vendedor.';
