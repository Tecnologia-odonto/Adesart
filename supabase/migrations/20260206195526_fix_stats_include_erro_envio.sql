/*
  # Corrigir contagem de estatísticas para incluir erro_envio como pendentes
  
  1. Problema Identificado
    - A função get_cadastros_stats estava contando apenas status 'incompleto' e 'enviado'
    - Registros com status 'erro_envio' não eram contabilizados em nenhum grupo
    - Isso causava discrepância: Total ≠ Pendentes + Cadastrados
  
  2. Solução
    - Modificar a função para incluir 'erro_envio' na contagem de incompletos/pendentes
    - Registros com erro_envio precisam ser reprocessados, portanto são pendentes
  
  3. Impacto
    - Campo "Pendentes" agora incluirá registros com status 'incompleto' OU 'erro_envio'
    - Total = Pendentes + Cadastrados (correção da consistência)
*/

-- Dropar função anterior
DROP FUNCTION IF EXISTS get_cadastros_stats(uuid);

-- Criar função get_cadastros_stats com erro_envio incluído nos pendentes
...
BEGIN
  ...
  IF v_user_role IN ('ADMINISTRADOR', 'GERENTE') THEN
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE status IN ('incompleto', 'erro_envio')),
      COALESCE(SUM(
        CASE 
          WHEN status IN ('incompleto', 'erro_envio') AND dependentes IS NOT NULL AND jsonb_typeof(dependentes) = 'array'
          THEN jsonb_array_length(dependentes)
          ELSE 0
        END
      ), 0),
...
-- Comentários
COMMENT ON FUNCTION get_cadastros_stats IS 'Retorna estatísticas detalhadas de cadastros do mês atual separadas por tipo_cadastro (cadastro e inclusao_dependente). Registros com erro_envio são contabilizados como pendentes/incompletos.';
